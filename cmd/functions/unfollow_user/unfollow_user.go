...
func Handler(context context.Context, request events.APIGatewayProxyRequest, userId uuid.UUID) events.APIGatewayProxyResponse {
	...
	if err != nil {
		return api.ToErrorAPIGatewayProxyResponse(context, err, "UnFollowUserHandler")
	}

	return api.ToSuccessAPIGatewayProxyResponse(context, result, "UnFollowUserHandler")
}
